...
import {
  ...
  shortAddress, shortWorldId, transferAdminOwnership, withdrawAdminFee,
...
function worldIdLabel(policy: AdminPolicy) {
  return shortWorldId(policy.worldIdNullifierHash);
}
...
function PolicyList() {
  ...
  return <section className={card}><div className="mb-4 flex items-center justify-between"><div><h2 className="text-xl font-bold">Pólizas</h2><p className="text-sm text-[#a7b0c2]">{data.policyCount} emitidas</p></div></div><div className="overflow-x-auto"><table className="w-full min-w-[860px] text-left text-sm"><thead className="border-b border-[#293446] text-[#a7b0c2]"><tr><th className="p-3">ID</th><th className="p-3">Titular</th><th className="p-3">Referencia World ID</th><th className="p-3">Estado</th><th className="p-3">Principal</th><th className="p-3">Fallecimiento</th><th /></tr></thead><tbody>{data.policies.map((policy) => <tr className="border-b border-[#202a38]" key={policy.id}><td className="p-3 font-semibold">#{policy.id}</td><td className="p-3">{shortAddress(policy.holder)}</td><td className="p-3 font-mono text-[#d7def0]" title={policy.worldIdNullifierHash}>{worldIdLabel(policy)}</td><td className="p-3">{policyStatus[policy.status] ?? "Unknown"}</td><td className="p-3">{formatAdminUsdc(policy.principal)}</td><td className="p-3">{policy.deathReported ? "Reportado" : "—"}</td><td className="p-3"><Link className="text-[#aeb8ff]" href={`/admin/policy/${policy.id}`}>Ver</Link></td></tr>)}</tbody></table></div><p className="mt-3 text-xs text-[#95a3bc]">Referencia on-chain de la verificación World ID; el identificador privado no se expone.</p><div className="mt-5 flex items-center justify-between"><button className={button} disabled={page === 0} onClick={() => setPage(page - 1)}>Anterior</button><span className="text-sm text-[#a7b0c2]">Página {page + 1}</span><button className={button} disabled={(page + 1) * 20 >= data.policyCount} onClick={() => setPage(page + 1)}>Siguiente</button></div></section>;
}
...
function PolicyDetail({ policyId, account }: { policyId: string; account: string }) {
  ...
  return <section className="space-y-5"><Link className="text-sm text-[#aeb8ff]" href="/admin/policies">← Todas las pólizas</Link>{message && <p className="rounded-xl bg-[#172335] p-3 text-sm">{message}</p>}<div className={card}><div className="flex flex-wrap justify-between gap-4"><div><p className="text-sm text-[#a7b0c2]">Póliza #{policy.id}</p><h2 className="text-2xl font-bold">{shortAddress(policy.holder)}</h2></div><span className="rounded-full bg-[#20295b] px-3 py-1 text-sm text-[#c7ceff]">{policyStatus[policy.status]}</span></div><div className="mt-5 grid gap-4 sm:grid-cols-3"><Metric label="Principal" value={formatAdminUsdc(policy.principal)} /><Metric label="Abierta" value={date(policy.openedAt)} /><Metric label="Pagos" value={String(policy.payoutsMade)} /></div></div><div className={card}><h3 className="font-bold">Verificación World ID</h3><p className="mt-2 text-sm text-[#a7b0c2]">Wallet: {shortAddress(policy.holder)}</p><p className="mt-2 text-xs text-[#95a3bc]">Referencia de nulificador registrada on-chain:</p><p className="mt-2 break-all font-mono text-sm text-[#d7def0]">{policy.worldIdNullifierHash}</p><p className="mt-3 text-xs text-[#95a3bc]">World ID no expone el identificador personal de la persona. Esta referencia permite comprobar que la póliza fue autorizada por una verificación única.</p></div><div className={card}><h3 className="font-bold">Beneficiarios</h3><ul className="mt-3 space-y-2 text-sm">{policy.beneficiaries.map((beneficiary) => <li className="flex justify-between rounded-lg bg-black/15 p-3" key={beneficiary.account}><span>{beneficiary.account}</span><span>{beneficiary.shareBps / 100}%</span></li>)}</ul></div><div className={card}><h3 className="font-bold">Aviso de fallecimiento</h3><p className="mt-2 text-sm text-[#a7b0c2]">{policy.death.active ? `Reportado por ${shortAddress(policy.death.reporter)}. Reclamable: ${date(policy.death.claimableAt)}` : "Sin aviso activo."}</p><button className={`${button} mt-4`} disabled={working || policy.death.active} onClick={() => void report()}>{working ? "Confirmando…" : "Reportar fallecimiento"}</button></div></section>;
}
