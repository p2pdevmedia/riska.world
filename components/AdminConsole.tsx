...
function worldIdLabel(worldId: AdminPolicy["worldId"]) {
  if (!worldId) {
    return "—";
  }

  const primaryIdentifier = worldId.credentialIdentifiers[0];

  return primaryIdentifier ? shortWorldId(primaryIdentifier) : shortWorldId(worldId.nullifierHash);
}
