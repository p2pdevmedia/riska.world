...
export async function getTestnetPolicy({
  ...
  return {
    ...
    holder: policy[0],
    isViewerBeneficiary,
    lastHolderInteractionAt: Number(policy[5]),
    monthlyPayoutAmount: policy[9],
    monthlyPayoutEstimate,
    nextPayoutAt: Number(policy[6]),
    openedAt: Number(policy[4]),
    payoutsMade: Number(policy[3]),
    policyId,
    auxiliaryTokens,
    yieldPositions,
    yieldStrategies,
    remainingExtraPrincipal: policy[8],
    remainingMinimumPrincipal: policy[7],
    status: policy[10],
    termsHash: policy[2],
    totalPrincipal
  };
